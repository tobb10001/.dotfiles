# fish completion for regctl                               -*- shell-script -*-

function __regctl_debug
...
function __regctl_perform_completion
    __regctl_debug "Starting __regctl_perform_completion"
    ...
    __regctl_debug "Comps: $comps"
    __regctl_debug "DirectiveLine: $directiveLine"
    __regctl_debug "flagPrefix: $flagPrefix"
...
function __regctl_prepare_completions
    __regctl_debug ""
    __regctl_debug "========= starting completion logic =========="
    ...
    if test $compErr -eq 1
        __regctl_debug "Received error directive: aborting."
        # Might as well do file completion, in case it helps
        return 1
    end
    ...
    if test $filefilter -eq 1; or test $dirfilter -eq 1
        __regctl_debug "File extension filtering or directory filtering not supported"
    ...
    __regctl_debug "nospace: $nospace, nofiles: $nofiles"
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __regctl_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__regctl_comp_results)
        set --global __regctl_comp_results $completions
        __regctl_debug "Filtered completions are: $__regctl_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__regctl_comp_results)
        __regctl_debug "numComps: $numComps"
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__regctl_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                __regctl_debug "Adding second completion to perform nospace directive"
                set --global __regctl_comp_results $split[1] $split[1].
                __regctl_debug "Completions are now: $__regctl_comp_results"
            end
        end
        ...
        if test $numComps -eq 0; and test $nofiles -eq 0
            ...
            __regctl_debug "Requesting file completion"
            return 1
        end
    end
...
if type -q "regctl"
    ...
    complete --do-complete "regctl " > /dev/null 2>&1
end
